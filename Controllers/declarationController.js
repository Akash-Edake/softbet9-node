  ...
  try {
    ...
    if (currUser.parent_id && currUser.parent_id != '0') {
      ...
      try {
        ...
        setQuery.$inc["profit_loss_ledger." + match_id] = parseFloat(collectBalance);
...
var declareMatch = async (req, res) => {
  try {
    ...
    for (let i = 0; i < matchLogs.length; i++) {
      ...
      handleCollectionReport(matchLogs[i].user_id, totalWinnings, myobj.match_id);
...
var declareFancy = async (req, res) => {
  try {
    ...
    for (let i = 0; i < fancyTrans.length; i++) {
      ...
      //Updateting the User
      handleCollectionReport(currentFancy.user_id, totalWinnings,myobj.match_id);
      const user = await User.findByIdAndUpdate(
