...
const placeOddsBet = async (req, res) => {
  try {
    ...
    const user = await User.findById(myobj.user_id);
    console.log({ user_id: myobj.user_id, user });
    console.log({ exposure, credits: user.credits, user_exp: user.exposure,updatedPosition:updatedPosition });
    const finalExpo = Math.abs(exposure + (user.exposure || 0));
    if (Math.abs(finalExpo) > user.credits)
      return res
        .status(200)
...
const getMatchesByUserBet = async (req, res) => {
  try {
    ...
    // Prepare match data
    const matchData = Object.entries(matchUserMap).map(([match_id, betCount]) => ({
      match_id,
    ...
    const matchesWithCounts = matches.map((match) => ({
      ...
      user_count: matchData.find((m) => m.match_id === match.match_id)?.user_count || 0,
    }));
